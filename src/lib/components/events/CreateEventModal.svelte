<script lang="ts">
  ...
  import { isReadOnly } from '$lib/stores/auth';
  ...
  <div class="modal-box max-w-lg">
    ...
    {#if $isReadOnly}
      <div class="alert alert-warning mb-4">
        ...
        <div>
          <span>Creating events is disabled in read-only mode. </span>
          <a href="{base}/signup" class="link link-hover font-semibold underline">Complete signup</a>
          <span> to create events.</span>
        </div>
      </div>
      <div class="modal-action">
        <button type="button" class="btn btn-ghost" on:click={closeModal}>Close</button>
      </div>
    {:else}
