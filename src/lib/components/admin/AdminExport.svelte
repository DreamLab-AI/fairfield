  ...
  <div class="export-section">
    ...
    <div class="export-card">
      <div class="export-card-header">
        ...
        <div>
          <h4>Local Cache Export</h4>
          <p>Export complete local browser database (messages, channels, users). This is NOT a server-side backup.</p>
        </div>
      </div>
      <button class="btn btn-secondary" on:click={handleDatabaseBackup} disabled={isExporting}>
        {isExporting ? 'Exporting...' : 'Export Local Cache'}
      </button>
    </div>
  </div>
