...
>
  ...
  <!-- Message bubble -->
  <div
    class="max-w-[80%] sm:max-w-md relative px-4 py-2 rounded-2xl"
    class:bg-primary={isSent}
    class:text-primary-content={isSent}
    class:bg-base-300={!isSent}
    class:ring-2={isLongPressed}
    class:ring-primary={isLongPressed}
    style="transform: translateX({$offsetX}px); transition: transform 0.05s linear;"
  ...
  {#if showActions}
    <div
      class="fixed inset-0 z-40 bg-base-100/50 backdrop-blur-sm"
    ...
    <div
      class="absolute z-50 bg-base-200 rounded-xl shadow-xl p-2 min-w-[180px]"
      class:left-0={!isSent}
      class:right-0={isSent}
      style="top: -4rem;"
